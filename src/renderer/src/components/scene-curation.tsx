...
import { randomSeed, useGenerationStore } from '../stores/generation-store'
...
export function SceneCuration({ onClose }: { onClose: () => void }): React.JSX.Element {
  ...
  const loadedSceneId = useRef<number | null>(null)
  ...
  const reload = useCallback(async (id: number, resetIndex: boolean): Promise<void> => {
    ...
    if (sceneIdRef.current !== id || version !== reloadVersion.current) return
    loadedSceneId.current = id
    setImages(items)
  ...
  async function deleteAndNext(): Promise<void> {
    if (deletingRef.current || loading || loadedSceneId.current !== sceneId) return
    ...
    try {
      ...
      if (idx >= 0 && idx < list.length - 1) {
        setImages([])
        setLoading(true)
        setSceneId(list[idx + 1].id)
      } else toast('마지막 씬입니다', 'info')
    } catch {
      toast('삭제에 실패했습니다. 현재 씬을 확인해주세요', 'error')
    } finally {
  ...
  /** 선별 없음 — 현재 씬 이미지를 전부 삭제하고 바로 다음 씬으로 이동 */
  async function deleteAllAndNext(): Promise<void> {
    if (sceneId == null || deletingRef.current || loading || loadedSceneId.current !== sceneId)
      return
    deletingRef.current = true
    setDeleting(true)
    const currentSceneId = sceneId
    const list = useScenesStore.getState().scenes
    const idx = list.findIndex((s) => s.id === currentSceneId)
    const next = idx >= 0 ? list[idx + 1] : undefined
    try {
      const { deleted, error } = await window.nais.invoke('scenes:clearCurationImages', {
        sceneId: currentSceneId
      })
      void useGenerationStore.getState().refreshHistory()
      void useScenesStore.getState().load()
      if (error) {
        await reload(currentSceneId, true)
        toast(error, 'error')
        return
      }
      ...
      if (next) {
        setLoading(true)
        setSceneId(next.id)
        toast(`${deleted}장 모두 삭제됨 · 다음 씬으로 이동`, 'success')
      } else {
        toast(`${deleted}장 모두 삭제됨 · 마지막 씬입니다`, 'info')
      }
    } catch {
      toast('전체 삭제에 실패했습니다. 현재 씬을 확인해주세요', 'error')
    } finally {
      deletingRef.current = false
      setDeleting(false)
    }
  ...
  return (
    ...
    <div className="no-drag flex min-h-0 flex-1 flex-col bg-black/60 p-3 backdrop-blur-sm">
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-line bg-paper shadow-2xl">
        {/* 헤더 — 제목 + 레퍼 설정 행(한 줄 유지, 좁으면 줄바꿈) + 선별 작업 + 닫기 */}
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 border-b border-line px-3 py-2">
          ...
          <Button
            ...
            disabled={
              images.length === 0 || loading || deleting || loadedSceneId.current !== sceneId
            }
            onClick={() => void deleteAllAndNext()}
            title="이 씬의 이미지를 전부 삭제하고 다음 씬으로 이동"
          >
            <Trash2 size={14} /> 선별 없음
          </Button>
          <Button
            size="sm"
            variant="ghost"
          ...
          >
            ...
            {reference ? (
              <div className="relative min-h-0 flex-1">
                <img
                  src={imageUrl(reference.filePath, imageRevisions[reference.filePath])}
              ...
              <>
                <ContextMenu>
                  <ContextMenuTrigger asChild>
                    <div className="relative min-h-0 flex-1">
                      <img
                        src={imageUrl(current.filePath, imageRevisions[current.filePath])}
                        className="absolute inset-0 h-full w-full object-contain p-2"
                        draggable={false}
                        alt=""
                      />
                  ...
                  <ContextMenuContent>
                    ...
                    <ContextMenuItem onSelect={() => openCensor(current.filePath)}>
              ...
              <div className="min-h-0 flex-1 overflow-y-auto p-1">
                {scenes.map((s) => (
                  <button
                    ...
                    disabled={deleting}
                    onClick={() => {
                      if (!deletingRef.current) setSceneId(s.id)
                    }}
                    className={cn(
                      ...
                      <img
                        src={
                          img.thumbnail
                            ...
                            : imageUrl(img.filePath, imageRevisions[img.filePath])
                        }
